...
temp1.meta <-
  temp1 |>
  select(of_variable, unit, source) |>
  unique() |>
  filter(!is.na(unit)) |>
  bind_rows(
...
temp3 <- data.lvl0.df$data$Table3$primary |>
  pivot_longer(cols = -variable,
               names_to = 'elevation_id', values_to = 'with_entry',
               values_drop_na = TRUE) |>
  mutate(`elevation_id` = str_extract(elevation_id, '\\d{3,} m')) |>
  pivot_wider(names_from = 'variable', values_from = 'with_entry') |>
  mutate(`Elevation (m)` = str_extract(elevation_id, '\\d{3,}')) |>
  pivot_longer(cols = -elevation_id, names_to = 'column_name', values_to = 'with_entry') |>
...
temp3.meta <- temp3 |>
  ...
  pivot_longer(cols = -c(column_name, of_variable),
