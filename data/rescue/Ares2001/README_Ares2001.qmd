...
#semantic files
of_variable.file <- '../../semantics/SoilDRaHVocabulary_is_type.csv'
...
```{r}
#| label: createInitalSemanticVocab
#| eval: false
#| echo: false

tibble(of_variable = c(data.lvl1.ls$meta$of_variable, data.lvl1.ls$primary$of_variable) |> unique() |>sort(),
       definition = ' ',
       status = ' ') |>
  write_csv(file = '../../semantics/SoilDRaHVocabulary_of_variable.csv', quote = 'all')

data.lvl1.ls$meta |> select(of_variable, is_type) |> unique() |>
  bind_rows(data.lvl1.ls$primary |> 
              select(of_variable, is_type) |> unique()) |>
  filter(!(str_detect(of_variable, 'A_koa') | str_detect(of_variable, 'F_uhdei'))) |>
  arrange(of_variable, is_type) |>
  reframe(of_variable = paste(of_variable, collapse = ','),
          .by = is_type) |>
  write_csv(file = of_variable.file, quote = 'all')
```
...
SoilDRaH_variable_list <- read_csv(file = '../../semantics/SoilDRaHVocabulary_of_variable.csv',
