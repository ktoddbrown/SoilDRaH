...
study.df <- meta.df <- tribble(
  ...
  bind_rows(tribble(~of_variable, ~method, ~source,
                    ...
                    'sample prep', 'oven dried at 70 C, sieved <2mm', paste('Methods, ln27,29:', paste( data.lvl0.df$method[c(27, 29)], collapse = '\n')),
...
site.df <- tribble(
  ...
  'observation year', '1995', 'CE', 'In the discussion section @Bashkin1998 talks about expected total carbon changes over the 10-13 years of the study not being meet and since the sites were planted in 1982-1985, we assume samples were taken in 1995.', '@Bashkin1998',
  ## Pull in the site descriptions
  'soil classification', 'isothermic Typic Hydrudands of the Akaka and Kaiwiki series', NA, 'USDA', paste('Methods, ln8:', data.lvl0.df$method[8]),
  'minerology', 'amorphous noncrystalline hydrous alumina and iron oxides', NA, NA, paste('Methods, ln9:', data.lvl0.df$method[9]),
  'parent material', 'volcanic ash', NA, NA, paste('Methods, ln9:', data.lvl0.df$method[9])
) 
...
#### Landuse ####
#### Process the landuse documented in Table 1
landuse <- data.lvl0.df$data$Table1$primary |>
  mutate(management_history = str_extract(data.lvl0.df$data$Table1$caption, '†.*(?=\\r\\n)'),
         paired_wildland = ifelse(str_detect(`Site †`, '‡'),
                                  str_extract(data.lvl0.df$data$Table1$caption, '‡.*(?=\\r\\n)'),
                                  NA_character_),
...
soilObs <- data.lvl0.df$data$Table3$primary |>
...
total_carbon.meta <- tibble(of_variable = 'total_carbon',
  ...
  full_join(soilObs |>
              ...
              select(`Depth (cm)`, landuse_id, of_variable) |>
              unique(),
...
soilObs <- soilObs |>
  ...
  bind_rows(total_carbon.meta)

meta.df <- study.df |>
  bind_rows(site.df) |>
  ##Turn this into a long data frame
...
meta.df <- pivotBind |>
  filter(of_variable %in% c('bulk_density', 'organic_fraction',
                            'sample_design', 'core_description', 'sample prep')) |>
  select(study_id,
...
site_history <- pivotBind |>
  ...
  mutate(`Eucalyptus plantation::duration` =
           as.numeric(wideTable$`observation year::value`) - `Eucalyptus plantation::start`,
         `Eucalyptus plantation::prior_Cane_duration` = `sugar cane cultivation::end` - `sugar cane cultivation::start`,
         `sugar cane cultivation::duration` = as.numeric(wideTable$`observation year::value`) - `sugar cane cultivation::start`)
