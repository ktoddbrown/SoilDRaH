...
variable_keep <- c('variable_keep', 'download.dir')
```
...
variable_keep <- c(variable_keep, 'lvl0_data.ls')
...
```{r}
#remove the cores that have been replaced
#map the of_variables to a common vocabulary
#move column is_type up to study if needed
#map column names uniquely and replace the index
```
