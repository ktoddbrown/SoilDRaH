...
The final format of the data frame here will include the following 
  ...
  + `of_variable`:
    ...
    * bulk_density:
      + `is_type`:
        ...
        - unit
        - value
    * bulk_density_estimated_fine_earth:
      + `is_type`:
        - ISCN_vocabulary
        - description
        - name
 ...
 <!--- TODO we are experimenting with some new variable names here and will need to go back and revise the vocabulary. --->
...
site_geocluster <- site_org.df |>
  ...
  mutate(geolocation_id = sprintf('geocluster_%05d',
           ...
           hclust() |>
           cutree(h = 0.001))) #|> #Using euclidean distance, corresponds to ~100m
  #filter(n() > 1, .by = geolocation_id)

# temp <- dist(matrix(c(site_geocluster$`lat (dec. deg)`, site_geocluster$`long (dec. deg)`), ncol = 2)) |>
#            hclust()
#summary(temp$height < 0.0001) #5443 unique sites
#summary(temp$height > 0.001) #5117 unique sites
#summary(temp$height < 0.01) #4205 unique sites
site_temporalCluster <- site_org.df |>
  ...
  mutate(date_year = year(observation_date),
         temporal_id = sprintf('timecluster_%02d',
                               dist(matrix(decimal_date(observation_date), ncol = 1)) |> 
           hclust() |>
           cutree(h = 2))) #max height here corresponds to year span, consider two years to be effectively the same observation time
...
There are `{r} uniqueSpaceTime` number of unique geolocation-time pairs reported.
...
layer.df <- data.lvl0.ls$data$TableLayer$primary |>
  ...
  filter_out(if_all(everything(), is.na)) |> #does not remove columns right now
  left_join(site.df,
            by = join_by(dataset_name_sub, dataset_name_soc, site_name, profile_name)) |> 
  mutate(across(c(`layer_top (cm)`, `layer_bot (cm)`), as.numeric)) |>
  #layer resolution should not drop below 1-cm so construct the identifier from the unique combinations of the top/bottom layers
  mutate(layer_id = case_when( 
    !is.na(`profile_zero_ref (profile_zero_ref)`) &
      !is.na(`layer_top (cm)` + `layer_bot (cm)`) ~ sprintf('layer (zero-ref:%s) [%d,%d]',
                                                            `profile_zero_ref (profile_zero_ref)`,
                                                            round(`layer_top (cm)`, digits = 0),
                                                            round(`layer_bot (cm)`, digits = 0)),
    is.na(`profile_zero_ref (profile_zero_ref)`) &
      !is.na(`layer_top (cm)` + `layer_bot (cm)`) ~ sprintf('layer [%d,%d]',
                                                            round(`layer_top (cm)`, digits = 0),
                                                            round(`layer_bot (cm)`, digits = 0)),
    !is.na(`profile_zero_ref (profile_zero_ref)`) &
      is.na(`layer_top (cm)` + `layer_bot (cm)`) ~ sprintf('layer (zero-ref:%s)',
                                                            `profile_zero_ref (profile_zero_ref)`),
    is.na(`profile_zero_ref (profile_zero_ref)`) &
