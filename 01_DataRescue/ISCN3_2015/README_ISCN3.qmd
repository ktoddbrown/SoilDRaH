...
AnnotationsDir <- '.'
ISCN3_layerAnnotations <- 'ISCN3_layer.csv'
ISCN3_profileAnnotations <- 'ISCN3_profile.csv'
ISCN3_siteAnnotations <- 'ISCN3_site.csv'
ISCN3_disturbanceAnnotations <- 'ISCN3_disturbance.csv'
ISCN3_fractionAnnotations <- 'ISCN3_fraction.csv'
...
```{r}
knitr::kable(iscn.ls$annotations$layerAnnotations |> 
               head()) |>
  kable_paper() |>
  scroll_box(width = "100%", height = "300px")
...
Bibliography <- list(primary = NULL, contributed = NULL)
  if(!is.null(primaryCitation.file) && file.exists(primaryCitation.file)){
    Bibliography$primary <- bibtex::read.bib(primaryCitation.file)
  }

  if(!is.null(contributionsCitation.file) && file.exists(contributionsCitation.file)){
    Bibliography$contributed <- bibtex::read.bib(contributionsCitation.file)
  }
```
