...
### CONSTRUCTING ANNOTATIONS (machine readable)
...
disturbance_table <- big_stuff[737:829,]
colnames(disturbance_table) <- as.character(unlist(disturbance_table[1, ]))
disturbance_table <- disturbance_table[-1, -6]
names(disturbance_table)[3:5] <- c("Units", "Description", "Page_number")

collapsed_disturbance_table <- disturbance_table |> 
  group_by(Variable) |> 
    summarise(Name = collapse_tokens(Name),
              Units = collapse_tokens(Units),
              Description = collapse_tokens(Description),
