...
geolocation.df <- layer.df |>
  ...
  pivot_longer(cols = c(latitude, longitude, resolution, observation_count, datum),
...
organic_fraction.df <- layer.df  |>
  ...
  mutate(value = round(as.numeric(`loi (percent)`), 
                       digits = precisions.ls$organicFraction_digits)) |>
  #Deal with duplicate values
  reframe(sample_id = ifelse(n() == 1,
                            ...
                            paste0('[MultiSourced]',
                                   paste(sort(unique(sample_id)), 
                                         collapse = ';'))),
         .by = c(geolocation_id, temporal_id, layer_id, value)) |> #553 obs
...
4) Some layers have multiple observations making their layer/sample identifiers non-unique for these observations.
  - *To resolve* extend the sample_id to include a row index.
...
carbon_fraction.df <- layer.df  |>
  ...
  mutate(sample_id = ifelse(n() > 1,
                            paste0(sample_id, ';row:', 1:n()),
                            sample_id),
         .by = c(geolocation_id, temporal_id, layer_id, of_variable)) |>
...
bulkDensity_methodMap <- tribble(~column_name_bd, ~bd_method, ~bdNRCS_prep_code, ~what, ~how,
...
bulkdensity.df <- layer.df  |>  #72 503 obs
  ...
  left_join(bulkDensity_methodMap,
            by = join_by(bd_method, bdNRCS_prep_code, column_name_bd)) |>
  mutate(of_variable = str_extract(what, pattern = '^\\w+')) |>
  mutate(method = if_else(is.na(how),str_remove(what, pattern = '^\\w+-'),
                          paste(str_remove(what, pattern = '^\\w+-'), how, sep = '-'))) |> #59 455 obs
  ...
  reframe(sample_id = paste0('[MultiSourced]', paste(unique(sample_id), collapse = ';')),
         ...
         .by = c(geolocation_id, temporal_id, layer_id, #IDs
                 of_variable, value_bd)) |> #49 974 obs, roughly 15% removed as duplicate values
  select(ends_with('_id'),
         dataset_name, #keep the dataset name for citation cross reference
         value = value_bd, method) |>
  mutate(of_variable = 'bulk_density', unit = 'g cm-3') |>
  #there are some layers with different types of bulk density
  #...assign an observation ID to preserve grouping before pivoting longer
  mutate(observation_id = paste(of_variable, 1:n(), sep = ':')) |>
  #convert thingss to a string and ensure correct precision
  mutate(value = sprintf(paste0('%.',precisions.ls$bulkDensity_digits,'f'), 
                         value)) |>
