...
bulkdensity.df <- layer.df  |>
  ...
  select(geolocation_id, temporal_id, layer_id, sample_id,
         column_name_bd, value_bd, method_bd) |> #59 459
...
organic_fraction.df <- layer.df  |>
  select(ends_with('_id'), `loi (percent)`)  |>
  ...
  mutate(sample_id = ifelse(n() == 1 | length(unique(sample_id)) == 1,
         ...
         .by = c(geolocation_id, temporal_id, layer_id, value)) |> 
  unique() |> #936
  select(geolocation_id, temporal_id, layer_id, sample_id, value) |>
...
```{r}
depth.df <- layer.df  |>
  select(dataset_name = dataset_name_sub, site_name, #cross with the site dataframe
...
#depth.df already here

geolocation.df <- geolocation.df |>
  bind_rows(region.df)

#temporal.df already here
# need to pull citations
```
