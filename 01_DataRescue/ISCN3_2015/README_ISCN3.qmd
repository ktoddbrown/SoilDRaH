...
The code is also in `{r} readsource.file` and returned with the `dataLevel = 'level0'` argument.
Below verifies that the data from the function matches what is coded above.

```{r}
#| label: checkReadFunction_lvl0

if(file.exists(readsource.file) & FALSE){
...
#this is nto exactly clean but I think it's workable
  layer.meta <- layer.df |>
    ...
    select(column_header, of_variable, 
           ISCN_vocabulary = ISCN_variable, name = Name, unit = Units, description = Description) |>
    pivot_longer(cols = c(ISCN_vocabulary, name, unit, description),
