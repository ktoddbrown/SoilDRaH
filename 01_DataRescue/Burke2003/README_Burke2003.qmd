...
author:
  ...
  - id: SavaScott #use the github name as an id
    ...
    #  - transcription:  #secondary, review
      - standardization: primary #taking over for Mirah
affiliations:
  - id: uf-ees
...
- Location: Sites from Hawaii, Brazil, and Ecuador are included, with two of them being in Hawaii. No latitude and longitude coordinates are listed, but regional descriptions are given in methods section. Information about mean annual precipitation, temperature, and sampling depth at sites given.
- Soil carbon stock: SOM stock presented in Table 1, with depth-specific organic carbon values.
- Soil type: Classification and extra soil history information is given in the methods section (Typic Distrandepts). Additional soil properties such as pH, organic matter content, and microbial biomass are reported in figures and tables.
- Land use: Hawaii Sites include sugarcane and pasture, further description and history is provided in the methods section.
...
data.lvl1.ls <- list(
  ...
  primary = bind_rows(Table1Primary) |>
    mutate(location_id = with_entry[of_variable == 'region_id'],
           is_type = 'value',
           .by = row_id) |>
    arrange(row_id, location_id, column_name,
...
if(file.exists(readsource.file)){
  datafunction.lvl1.ls <- readBurke2003(dataDir = '.',
                                         dataLevel = 'level1')
