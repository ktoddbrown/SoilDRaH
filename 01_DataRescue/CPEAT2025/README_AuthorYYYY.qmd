...
#transfer between the old url list and the new bib files
##### Long doi list ####

doi.df <- tibble(dataURL = read_csv(file = 'download_url_CPEAT.csv', col_names = FALSE, col_types = cols(col_character()))$X1) |>
  mutate(doi = str_extract(dataURL, '10.*(?=\\?format)')) |>
  mutate(citationURL = paste0(str_extract(dataURL, 'https.*(?=\\?format)'), '?format=citation_bibtex'),
         id = str_extract(doi, '\\d+$'))

for(ii in 1:nrow(doi.df)){
  if(!file.exists(file.path('temp', 'orginalBibs', paste0(doi.df$id[ii],'.bib')))){
    download.file(url = doi.df$citationURL[ii],
                  destfile = file.path('temp', 'orginalBibs', paste0(doi.df$id[ii],'.bib')))
  }
}

#cat 8*.bib 9*.bib >> _temp.bib
#cat *.bib >> ../../CPEAT2025_Methods.bib
#Open with BibDesk and regenerate citation key to remove duplicates
```
