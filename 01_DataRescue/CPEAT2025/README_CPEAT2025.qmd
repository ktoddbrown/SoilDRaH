...
Some of these DOIs have been superseded and are flagged as 'replaced by'.
All DOIs are listed in the methods BibTex file and can be compared to the `pangaear::pg_search` code in the `checkDOIs` chunk that is hidden in the output.
...
Pangaea files have graphical metadata appearing before a primary data table in each data download.
We process this information into three data tables: a `study` table containing the table-formatted metadata that is not information about the columns of the primary data, a `columns` table contianing the units and method notes for the primary data, and a `primary` table contiaining the layer level information.
These data tables are provided in a `<id>-of_variable-is_type-with_entry` tuple.

- **Location**: The geo-location is header information and ends up in the `study` table of the processed level 0 data.
- **Soil carbon stock**: Soil carbon stock, total organic carbon fraction, organic carbon fraction, and loss on ignition are all included in the primary data table and ends up in the `primary` table of the level 0 data. Coarse fraction was not reported and does not appear to be included.
  * **Layer depth**: Depth of surveyed soil layer is in the `primary` table.

## Files

<!--- Include a brief explaination to all the files in the folder--->

These files are in the CPEAT2025 data rescue.

- [Readme](README_CPEAT2025.qmd)
  + This is the primary file that documents the transcriptions and decision made during data rescue.
- [CPEAT2025.bib](CPEAT2025.bib)
  + citation for project website and primary journal article transcribed
- [CPEAT2025_Methods.bib](CPEAT2025_Methods.bib)
  + citations for the cores archived on Pangaea that make up the project
- [CPEAT2025_Methods.md](CPEAT2025_Methods.md)
  + transcription of the project website
- [temp/](temp/)
  + the download location for CPEAT core files archived on Pangaea
...
if(any(download.flag)){
  message("This download may take some time.")
...
#put it all together
lvl0_dataTables <- list(study = study_meta.df,
...
Put everything together with the primary citation and text, as well as the three main data tables into a level 0 list.
...
data.lvl0.ls <- list(
  ...
  data = lvl0_dataTables
)

```

Finally make sure everything was copied over to the read function and the output matches the code documented here.
...
semantics.df <- datafunction.lvl1.ls$data$core |>
  select(of_variable = of_variable_DRaH, is_type) |>
  unique() |>
  filter_out(is.na(of_variable) | is.na(is_type)) |>
  bind_rows(
    datafunction.lvl1.ls$data$variable |>
      select(of_variable = of_variable_DRaH, is_type) |>
      unique() |>
      filter_out(is.na(of_variable) | is.na(is_type))
  ) |>
  bind_rows(
    datafunction.lvl1.ls$data$primary |>
      select(of_variable = of_variable_DRaH, is_type) |>
      unique() |>
      filter_out(is.na(of_variable) | is.na(is_type))
  ) |>
  mutate(site_id = 'CPEAT2025') |>
  arrange(of_variable) 
