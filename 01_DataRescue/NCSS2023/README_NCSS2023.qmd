...
rm(lvl0.primary) #remove large file
```
...
# bulk_density <- data.lvl0.ls$data$lab_method_code |>
#   filter(str_detect(requested_anal_name, '[Bb]ulk [Dd]ensity')) |>
#   filter_out(str_detect(requested_anal_name, '[Bb]ar') |
#                str_detect(requested_anal_name, '[Mm]oist') |
#                str_detect(requested_anal_name, 'Rewet'))
...
#TODO 1) pull column descriptions and 2) deal with merging tables w/ different keys

#join the soil observations

soilObs.df <- physical.df |>
  bind_rows(chemical.df)

meta.df <- data.lvl0.ls$meta$columns |>
  semi_join(soilObs.df,
