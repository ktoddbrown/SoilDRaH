...
bulk_density <- data.lvl0.ls$data$lab_method_code$lab_method_code |>
...
#pull bulk density and coarse fraction
physical.df <- data.lvl0.ls$data$lab_physical_properties$lab_physical_properties |>
  select(layer_key, #key across tables, lab_layer
         #result_source_key, row_id = objectid_1, duplicate of layer_key
         labsampnum, #unique id for sample
         prep_code, #lab_prep table
         bulk_density_oven_dry,
         bulk_density_oven_dry_method, #*_method go to lab_method_code
  ...
  filter(is.finite(bulk_density_oven_dry) |
           is.finite(bulk_density_lt_2_mm_air_dry) |
           total_frag_wt_pct_gt_2_mm_ws)

chemical.df <- data.lvl0.ls$data$lab_chemical_properties$lab_chemical_properties |>
  select(layer_key, #key across tables, lab_layer
         #result_source_key, row_id = objectid_1, duplicate of layer_key
         labsampnum, #unique id for sample
         prep_code, #lab_prep table
  ...
  filter(is.finite(total_carbon_ncs-caco3_lt_2_mm) |
           ...
           is.finite(organic_carbon_walkley_black))

layer.df <- data.lvl0.ls$data$lab_layer$lab_layer |>
...
pedon.df <- data.lvl0.ls$data$lab_pedon$lab_pedon |>
  select(pedon_key, site_key, pedlabsampnum,
         observation_date) |>
...
location.df <- data.lvl0.ls$data$lab_site$lab_site |> 
  select(site_key, user_site_id, horizontal_datum_name,
         latitude_std_decimal_degrees, longitude_std_decimal_degrees) |>
  filter(site_key %in% c(pedon.df$site_key, layer.df$site_key))
