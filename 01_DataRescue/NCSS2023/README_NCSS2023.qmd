...
location.df <- data.lvl0.ls$data$lab_site |> 
  select(site_key, user_site_id, horizontal_datum_name,
         latitude_std_decimal_degrees, longitude_std_decimal_degrees) |>
  filter(site_key %in% c(pedon.df$site_key, layer.df$site_key)) |> #53603 obs
  full_join(pedon.df, by = join_by(site_key)) #53662 obs

soilObs.df <- layer.df |>
  full_join(chemical.df)
