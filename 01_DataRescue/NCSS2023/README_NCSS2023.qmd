...
Since this is a large database which is served from the goverment website we duplicate the metadata here but do not include the primary date.
...
We also need `lab_pedon`, `lab_preparation`, `lab_method_code`, `lab_analyte` and `lab_analysis_procedure` based on the table above  and some light exploration.
...
# lvl0.primary <- apply(table_names, 1, 
#                       lab_area = 
#                        RSQLite::dbReadTable(myconnect, 'lab_area')
