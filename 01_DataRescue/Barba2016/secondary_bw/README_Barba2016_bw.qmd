...
bibliography:
  - Barba2016_Methods_bw.bib #contains the citations in the methods section
  - Barba2016_bw.bib #contains citation for the data source, generally a single citation of the primary paper
...
methods.file <- "Barba2016_Methods_bw.md"
Table2.file <- 'Barba2016_Table2_bw.csv'
figure4.file <- 'Barba2016_Figure4.csv'

# Bibliography files
primaryCitation.file <- 'Barba2016_bw.bib'
methodsCitation.file <- 'Barba2016_Methods_bw.bib'
...
readsource.file <- '../../02_DataHarmonization/readBarba2016.R'
...
For the discussion of this data rescue see the Github issue: [#252](https://github.com/ktoddbrown/SoilDRaH/issues/252).

This paper examines how drought-induced forest die-off and subsequent tree species replacement of _Pinus sylvestris_ L. (Scots pine) by _Quercus ilex_ L. ssp. _ilex_ (Holm oak) affect above and below ground litter decomposition in a mixed Mediterranean forest in the Prades Mountains, Spain.

## Tables and figures

- Table 
  1) Table 1: Results of the general linear model (GLM) testing for the effects of litter and microsite, and their interaction, on the decomposition constant ($\text{Yr}^{-1}$)
  2) Table 2: Initial litter quality on the study site

- Figure 
  1) Figure 1: Decomposition constants (k) of *Pinus sylvestris* and *Quercus ilex* litter (leaves and fine roots) across three habitats obtained by the GLM.
  2.) Figure 2:  Effects of litter type (a) and habitat (b) on the decomposition constant (k) (mean ± SE) obtained by the GLM.
  3.) Figure 3: Relationship between the initial C:N ratio of litter (leaves and fine roots from Pinus sylvestris and Quercus ilex)and decomposition constant (k) (mean ± SE), calculated as the mean of k considering the three habitat types
  4.) Figure 4: Mass remaining (mean ± SE) for each litter type (a) and habitat type (b) at 6 collection times (0.16, 0.5, 1, 1.5, 2 and 3 years, *n*=5). The lower case indicates significant differences in remaining mass between litter types (panel a) within collection times. 
...
Location: Titllar Valley, Prades Mountains, Northeast Iberian Peninsula, Spain (41°13′ N, 0°55′ E; 1015 m above sea level)
Duration: Litter bags were placed in July 2011 and pulled at intervals of 0.16, 0.5, 1, 1.5, 2, and 3 years.
Mass Loss: Percent of remaining mass can be found in Figure 4
Solar Exposure: Could not find solar exposure in this paper

### Other Relevant Variables:
Litter Species: 2 primary tree species were evaluated, Pinus sylvestris L. (Scots pine) and Quercus ilex L. ssp. ilex (Holm oak)
Microsite Data: Section Materials and Methods
Litter Chemical Composition: Initial carbon (%C) concentrations, nitrogen (%N) concentrations, and C:N ratios are in Table 2.
Litter Bag Materials and Methods: The litter bags were a size 7.5×8.5 cm, and made using a nylon mesh with holes of 0.5 mm. The litter bags included a mass of 0.5-1 g dried plant litter material.
Land Management: Mixed forest not managed for the last 30 years.
  
## Files

<!--- Include a brief explaination to all the files in the folder--->

These files are in the Barba2016 data rescue.

- [Readme](README_Barba2016.qmd)
  + This is the primary file that documents the transcriptions and decision made during data rescue.
- [Barba2016.bib](Barba2016.bib)
  + citation for article transcribed
- [Barba2016_Methods.bib](Barba2016_Methods.bib)
  + citations for the methods section of the article
- [Barba2016_Methods.md](Barba2016_Methods.md)
  + methods transcribed from primary article
- [Barba2016_Table2.csv](Barba2016_Table2.csv)
  + table 2 from primary article with initial litter quality on the study site.
- [Barba2016_Figure4.csv](Barba2016_Figure4.csv)
  + Figure 4 from primary article with mass remaining (mean ± SE) for each litter type (a) and habitat type (b) at 6 collection times (0.16, 0.5, 1, 1.5, 2 and 3 years, n=5).
- [temp/](temp/)
...
+ Lead
  ...
  - [ ] Create a fresh fork of the primary repository https://github.com/ktoddbrown/SoilDRaH/tree/main, it might be useful to rename it 'SoilDRaH_Barba2016'
  - [ ] Create a copy of this template folder and rename it to Barba2016
...
+ Do
  - [ ] Create a spreadsheet with Table2 and export it to csv
  - [ ] Extract points from Figure4 and export it to csv
...
diff --strip-trailing-cr --suppress-common-lines -y Barba2016_Table2_bw.md Barba2016_Table2.md
```
...
# You will want to repeat the line below to cross check each csv and md file in the
#... data rescue. Having no lines flagged by this command will ensure that each character matches exactly.
diff --strip-trailing-cr --suppress-common-lines -y Barba2016_Methods_bw.md Barba2016_Methods.md
```
...
if(file.exists(readsource.file)){
  datafunction.lvl0.ls <- readBarba2016(dataDir = '.',
                                         dataLevel = 'level0')
...
if(file.exists(readsource.file)){
  datafunction.lvl1.ls <- readBarba2016(dataDir = '.',
                                         dataLevel = 'level1')
