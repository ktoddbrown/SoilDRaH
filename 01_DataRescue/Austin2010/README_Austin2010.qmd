...
timelog:
  ...
  - activity: 
      ...
      duration: PT1H15M
  - activity:
      description: Start level 1 as class example
      who: ktoddbrown
      start: 2026-07-07 T17:36
      end: 2026-07-07 T18:00
      duration: PT24M
  - activity:
      description: Walk through data frame transformation (class exercise)
      who: ktoddbrown
      start: 2026-07-9 T17:30
      end: 2026-07-9 T18:00
      duration: PT30M
  - activity:
      description: Walk through data frame transformation continuatin (class exercise)
      who: ktoddbrown
      start: 2026-07-16 T17:30
      end: 2026-07-16 T18:00
      duration: PT30M
  - activity:
      description: Final cleanup for level 1 (class exercise)
      who: ktoddbrown
      start: 2026-07-21 T17:30
      end: 2026-07-21 T18:00
      duration: PT30M
---
...
# read function
readsource.file <- '../../02_DataHarmonization/readAustin2010.R'
...
- [Austin2010_Figure1.csv](Austin2010_Figure1.csv)
...
This data is identified as a data source for the DeAD (Decomposition Across Drylands).
Broad classes of information that we are looking for include location, duration, mass loss, solar exposure and litter chemistry.

More specifically:

<!--- Include description of where the data is located in the source.--->

  - [X] Location: a perennial grassland in the province of Córdoba, Argentina (31° 04 S, 64° 31 W)
  - [X] Duration: 120 days
  - [X] Mass loss: Mass loss or decay rate or mass remaining of litter sample over duration
  - [X] Solar exposure: Litter treated with filters (see treatments below)
  - [X] Litter species: Senescent litter was gathered on site, and were perennial tussock grasses. Identified species were *Festuca hieronymi* and *Stipa trichotoma*
  - [X] Vegetation Coverage: Open Woodland
  - [X] Litter Chemistry: Found via standard acid detergent method:
      + cellulose and sugars, 49.1 ± 0.4%
      + hemicellulose, 37.3 ± 0.2%
      + lignin,7.3 ± 0.2%.
  - [X] Litter bag materials and method: 0.750 g of litter in an open-topped plastic polystyrene box measuring 8 cm wide × 13 cm long × 2.5 cm.
  - [X] Treatments: 
      + Plastic box placed in chicken wire cage to allow for free flow of air.
      + Rainout shelter
      + No contact with ground; wire cage suspended 30cm above ground.
...
temp <- data.lvl1.ls$meta |>
  select(of_variable, is_type) |>
  bind_rows(data.lvl1.ls$primary |>
              select(of_variable, is_type)
            ) |>
  unique() |>
  mutate(study_id = 'Austin2010')

#write_csv(temp, file = '../../03_DataCuration/DeAD/vocabulary_draft.csv')
ref_draft <- read_csv('../../03_DataCuration/DeAD/vocabulary_draft.csv',
...
if(file.exists(readsource.file)){
  datafunction.lvl1.ls <- readAustin2010(dataDir = '.',
                                         dataLevel = 'level1')
