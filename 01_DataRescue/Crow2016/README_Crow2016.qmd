...
timelog:
  - activity: 
      ...
      duration: PT20H
  - activity:
      description: fixed render errors in YAML and beyond
      who: ktoddbrown
      start: 2027-07-28
      duration: PT1H
---
...
methods.file <- "Crow2016_Methods.md"
table1.file <- 'Crow2016_Table1.csv'
table2.file <- 'Crow2016_Table2.csv'
table4.file <- 'Crow2016_Table4.csv'
figure3.file <- 'Crow2016_Figure3.csv'

# Bibliography files
primaryCitation.file <- 'Crow2016.bib'
methodsCitation.file <- "Crow2016_Methods.bib"

readsource.file <- '../../02_DataHarmonization/Crow2016.R'
...
These files are in the Crow2016 data rescue.
...
- [Crow2016_Table1.csv](Crow2016_Table1.csv)
  + Table 1 from primary article with elevation, climate, soil series, and stand age for paired pasture and eucalyptus plots.
- [Crow2016_Table2.csv](Crow2016_Table2.csv)
  + Table 2 from primary article with soil carbon stocks by bulk density and equivalent mass methods, biomass carbon, and percent change from pasture
- [Crow2016_Table4.csv](Crow2016_Table4.csv)
  + Table 3 from primary article with soil carbon concentration and bulk density by depth with statistical comparisons
- [Crow2016_Figure3.csv](Crow2016_Figure3.csv)
  + Figure 3 from primary article with soil carbon stocks by depth and mass for pasture vs. eucalyptus plots.
- [temp/](temp/)
  + Contains original transcriptions for comparison
...
Table 1 was not modified, however footers were moved to the caption.
...
diff --strip-trailing-cr --suppress-common-lines -y temp/Crow2016_Table1_AW.csv temp/Crow2016_Table1_SRS.csv

#read_lines('temp/Crow2016_Table1_AW.csv') == read_lines('temp/Crow2016_Table1_SRS.csv')
```
...
Table 2 was modified by moving the column-wide sub-tables to row notation and populating the pasture biomass with empty cells.
...
diff --strip-trailing-cr --suppress-common-lines -y temp/Crow2016_Table2_AW.csv temp/Crow2016_Table2_SRS.csv
...
Table 4 was modified by moving the subheader information into a single primary header.

```{r}
#| label: table4Check


read_csv(file = table4.file, #read in the specific file
         skip = 1, #skip the first line with the caption
         #read everything as a character, don't worry about formatting yet
         col_types = cols(.default = col_character())) |>
  kable(caption = read_csv(file = table4.file, #read in that file again but...
                           n_max = 1, #only read the first line
                           col_names = 'caption', #call it the caption
                           #read everything as a character
                           col_types = cols(.default = col_character()),
...
diff --strip-trailing-cr --suppress-common-lines -y temp/Crow2016_Table4_AW.csv temp/Crow2016_Table4_SRS.csv
...
read_csv(file = figure3.file, #read in the specific file
         skip = 1, #skip the first line with the caption
         #read everything as a character, don't worry about formatting yet
         col_types = cols(.default = col_character())) |>
  kable(caption = read_csv(file = figure3.file, #read in that file again but...
                           n_max = 1, #only read the first line
                           col_names = 'caption', #call it the caption
                           #read everything as a character
                           col_types = cols(.default = col_character()),
...
plot.df <- read_csv(file = figure3.file, #read in the specific file
  ...
  mutate(y_value = if_else(str_detect(y, '\\-'),
                           mean(as.numeric(unlist(str_split(y, '-')))),
                           as.numeric(y)),
         .by = everything())

ggplot(plot.df) +
  geom_point(aes(x=-1*y_value, y = x, color = location)) +
  geom_line(aes(x=-1*y_value, y = x, color = location)) +
  geom_errorbar(aes(x=-1*y_value, ymax = x + `standard error difference`,
                    ymin = x - `standard error difference`,
                    color = location)) +
  coord_flip() +
  facet_wrap(~`y axis label`, scale = 'free_y') +
  scale_x_continuous(breaks = -1*plot.df$y_value, labels = plot.df$y) +
  theme(axis.title.y = element_blank()) +
  labs(y= unique(plot.df$`x axis label`))
```
...
diff --strip-trailing-cr --suppress-common-lines -y temp/Crow2016_Figure3_AW.csv temp/Crow2016_Figure3_SRS.csv
...
diff --strip-trailing-cr --suppress-common-lines -y temp/Crow2016_Methods_AW.md  temp/Crow2016_Methods_SRS.md
...
data.lvl0.ls <- list(
  ...
  data = list(
    ...
    #Read in all the data, skipping the first row with the caption and read
    #...in the table as character. This element is a tibble (data.frame).
    primary = read_csv(file = table4.file,
                       col_types = cols(.default = col_character()),
                       skip = 1)
    ), #end Table 4
    Figure3 = list(
