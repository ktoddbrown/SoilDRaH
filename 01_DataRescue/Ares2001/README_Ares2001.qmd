...
source.df <- data.lvl1.ls$study |>
  ...
  select(study_id, of_variable, with_entry) |>
...
site_history.df <- data.lvl1.ls$study |>
  ...
  mutate(study_id = 'Ares2001',
         land_use_id = 'LU1', #only one site in the study
         land_use = 'Plantation') |>
  select(study_id, land_use_id, land_use, 
...
layer.df <- data.lvl1.ls$primary |>
  ...
  #is_type == value so it can be dropped, column name is processed and can be dropped
  ...
  mutate(column_name = paste0(of_variable,'::',is_type)) |>
  select(layer_id = row_id, site_id = elevation_id, column_name, with_entry) |>
  pivot_wider(names_from = column_name, values_from = with_entry) |>
  #layer information in methods for soil variables
