...
temp <- dplyr::bind_rows(Table1Meta, Table3Meta) |>
  pivot_wider(names_from = c(of_variable, is_type),
              values_from = c(from_source, with_entry),
                names_sep = '::') |>
  right_join(
    bind_rows(Table1Primary, Table3Primary)|>
  ...
  pivot_longer(cols = -c(row_id, elevation_id, column_name),
               names_to = "header", values_to = 'with_entry',
               values_drop_na = TRUE) |>
  separate_wider_delim(cols = header, delim = '::',
                       names = c('entry_type', 'of_variable', 'is_type')) |>
  pivot_wider(names_from = entry_type, values_from = with_entry)
