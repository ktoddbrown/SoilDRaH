...
mutate(lowerbound = ifelse(
  column_name == "Age (mo)" & with_entry == "Change", str_extract(with_entry, '^\\d+(?=-)'),
  str_extract(with_entry, '-?\\d+\\.?\\d*(?=\\s*to)')),
  upperbound = ifelse(column_name == "Age (mo)" & with_entry == "Change", 
                      str_extract(with_entry, '(?<=\\d-)\\d+$'),
                    #working on this string first  str_extract(with_entry, '')),
         mean = str_extract(with_entry, '^-?\\d*\\.?\\d+(?=\\s*\\()'),
         'standard error' = str_extract(with_entry, '(?<=\\()[0-9.]+(?=\\))'),
