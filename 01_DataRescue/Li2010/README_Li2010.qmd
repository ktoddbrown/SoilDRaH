...
fig2 <- fig2 %>% #modify fig2 and pass back in
  mutate(
    ...
    `graph label` = factor(`graph label`,
      levels = c("A", "B"), # A first and then B
      labels = c("0–10 cm", "10–25 cm")
    ),
    `x label` = factor(`x label`, levels = c("TC", "SOC4", "SOC3")), #change to factor to control order of bars
...
# plot figure
ggplot(fig2, aes(x = `x label`, y = y, fill = ecosystem)) +
  geom_col(
    position = position_dodge(width = 0.8),
    width = 0.7
  ) +
  geom_errorbar(
    aes(ymin = y, ymax = `error bar`),
    position = position_dodge(width = 0.8),
    width = 0.2
  ) +
  geom_text(
    aes(label = footnote, y = `error bar` + 2),
    position = position_dodge(width = 0.8),
    vjust = 0
  ) +
  ...
  scale_fill_manual(
    values = c("S. Forest" = "red", "Pastures" = "blue"),
    name = ""
  ) +
  labs(
    x = "", # no x label
    y = expression(Soil~C~pools~(Mg~ha^{-1})) #expression() for the exponent
  ) +
