...
temp <- tibble(text = str_extract(dataDictionary[453], 
  ...
  separate_wider_delim(cols = text,
                      ...
                      delim = regex(' {2,}')) |> 
  mutate(Soil_sample_method_code = c(rep("when SOILS_SAMPLE_METHOD_CD = 1 (P3 Soils)", 4),
                                     rep("when SOILS_SAMPLE_METHOD_CD = 2 (Interior Alaska pilot)", 4),
                                     rep("when SOILS_SAMPLE_METHOD_CD = 3 (Interior Alaska)", 6),
                                     rep("when SOILS_SAMPLE_METHOD_CD = 4 (Hawaii)", 5)))
