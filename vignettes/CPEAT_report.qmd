...
CPEAT.ls$long |>
  filter(is.na(of_variable) & is_type == 'header_id') |>
  select(table_name, header_id = with_entry) |>
  unique() |>
  arrange(table_name) |>
  mutate(of_variable = paste0('unknown', 1:n()),
         is_type=case_when(table_name == 'data' ~ 'value',
                           table_name == 'metadata$parameters' ~ 'description',
                           table_name == 'metadata' ~ 'comment',
                           TRUE~NA_character_),
         with_entry = '--') |>
  format_csv() |>
  cat()
...
CPEAT_soc <- CPEAT.ls$long %>% 
  ...
  filter(of_variable %in% 
           c(#variables that identify the source data
             'doi', "data_doi", "download_url", "data_local_filepath",
             "project", "related_citation", "core_name", "citation",
             "core_comment", "comment", "data_license",
             #geolocation of the core
             "latitude", "longitude", "region", "elevation", "elevation_end",
             "elevation_start", "bounded_coverage",
             # related to the sampling of the core
            "sampling_device",
            "core_penetration", "core_recovery",
            "peat_description",
             #depth related variables 
             "layer_mid", "layer_thickness",
            ...
            "total_inorganic_carbon", "inorganic_carbon",
