...
dataDir <- 'CPEAT'
dataAnnotations <- 'annotations_CPEAT3.csv'
```
...
CPEAT.df$long %>%  # 2,640,892 entries, 7 total columns
  slice_head(n=100) %>%
  knitr::kable() %>%
  kable_paper() %>%
...
CPEAT_soc <- CPEAT.ls$long %>% 
  ...
  mutate(new_table = case_when(
    ...
    of_variable %in% c(#depth related variables 
             ...
             "depth_mid","depth_top", "depth_bottom") ~ 'soil_depth',
  ...
  mutate(of_variable = case_when(str_detect(doi, '10.1594_PANGAEA.927849') &
                                   ...
                                   ~ paste0(of_variable, '_', column_id),
                                 TRUE ~ of_variable))
...
CPEAT_profile <- CPEAT_soc |>
  ...
  unique()  #remove duplicate lat/lon
  
CPEAT_units <- CPEAT_profile |>
  mutate(unit = str_extract(with_entry, '(?<=\\d)\\D+$')) |>
  filter(!is.na(unit)) |>
  select(doi, of_variable, unit) |>
  reframe(unit = unique(unit),
          .by = of_variable) #double check that we can drop the dois

CPEAT_profile <- CPEAT_profile |>
  mutate(with_entry = str_remove(with_entry, '\\D+$') |>
           as.numeric()) |>
  pivot_wider(names_from = of_variable, values_from = with_entry)
...
CPEAT_layer <- CPEAT_soc |>
  ...
  #filter(str_detect(with_entry, '[^(\\d|\\.|\\-)]'))
  mutate(with_entry = as.numeric(with_entry)) |> #two entries '?0' being replaced with NA
  pivot_wider(names_from=of_variable, values_from=with_entry) 

  

CPEAT_layer.meta <- CPEAT_soc |>
  filter(new_table %in% c("soil_depth", "bulk_density", "soil_carbon")) |>
  filter(is.na(observation_id)) |>
  select(doi, of_variable, is_type, with_entry) |>
