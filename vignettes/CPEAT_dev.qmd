...
read_oneFile <- function(zz, format = c('original', 'long')[1]){
  fileread <- read_lines(zz) |>
    paste(collapse = '\n')
...
tester <- llply(inputFiles, .fun = read_oneFile, format = 'original')
tester <- ldply(inputFiles, .id = 'doi', .fun = read_oneFile, format = 'long')
