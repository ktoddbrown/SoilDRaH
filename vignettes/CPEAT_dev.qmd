...
temp2 <- tester2 |>
  ...
  mutate(name_long = case_when(
    str_detect(`METHOD/DEVICE`, 'OxCal') ~ paste(name_long, '(OxCal 4.2.4)'),
    str_detect(`METHOD/DEVICE`, 'Bacon') ~ paste(name_long, '(Bacon 2.2)'),
    str_detect(name_long, 'Peat type') & !is.na(COMMENT) ~ paste0(name_long, ' (',  COMMENT, ')'),
    str_detect(name_long, 'Method comment') ~ COMMENT,
    str_detect(name_long, 'Age, uncertainty') &
      str_detect(COMMENT, 'Lab uncertainty') ~ paste0(name_long, ' (',  COMMENT, ')'),
    str_detect(name_long, 'Age, uncertainty') &
      str_detect(COMMENT, '(14C age)|(Age AD)') ~ paste0(name_long, ' (',  COMMENT, ')'),
    str_detect(name_long, 'DEPTH') &
      str_detect(COMMENT, '(LOI)|(Age)') ~ paste0(name_long, ' (',  COMMENT, ')'),
    .default = name_long)) |>
  #where is description text non-unique in the tables
  filter(n() > 1, .by = c(doi, name_long))
