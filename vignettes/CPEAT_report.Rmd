...
CPEAT_soc <- CPEAT.ls$long %>% 
  ...
  pivot_wider(names_from = column_number,
...
layer.df <- CPEAT_soc %>% 
  select(doi_primary = doi, table_name, row_number, of_variable, with_entry) %>% # 303, 742 obs of 7 variables
  pivot_wider(names_from = of_variable,
              values_from = with_entry,
              names_repair = "unique")
```
...
```{r}
ggplot(layer.df) +
  geom_point(aes(x = bulk_density, y = `TOC [%]`), alpha = 0.1)
```
