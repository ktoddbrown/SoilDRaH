...
#memory management
primary_variables <- c(primary_variables, 'ISCN3_lvl1')
...
FIA_controlVocab <- fia.ls$annotations |>
...
FIA_lvl1 <- list(
  primary = fia.ls$long |>
    #make sure to set the study_id
    mutate(study_id = 'FIA') |>
    #### pull in the target variable that we want to map ####
    right_join(fia.ls$annotation |>
                 mutate(study_id = 'FIA') |>
                 #pull the annotations for the primary data entries
                 right_join(ISCN4map,
                            by = join_by(study_id, table_id,
                 ...
                 select(ends_with('_id'), of_variable, target_variable, is_type),
               relationship = "many-to-many",
               by = join_by(column_id, table_id, study_id)) |>
    #### replace the control vocab with the provided text values ####
    left_join(FIA_controlVocab,
              by = join_by(table_id, column_id, of_variable, is_type, with_entry)) |>
    mutate(with_entry = if_else(is.na(keyed_value), with_entry, keyed_value)) |>
    select(-keyed_value) |>
    #### Kick out to wide format for QAQC ####
    select(study_id, starts_with('CN.'), #CN is the row identifier
           target_variable, is_type, with_entry) |>
    pivot_wider(names_from = c(target_variable, is_type), names_sep = '::',
                values_from = with_entry),
  
  meta = fia.ls$annotations |>
    mutate(study_id = 'FIA') |>
    #pull the annotations for the primary data entries
    right_join(ISCN4map,
               by = join_by(study_id, table_id,
...
#memory management
primary_variables <- c(primary_variables, 'FIA_lvl1')
rm(list = setdiff(ls(), primary_variables)) 
```
...
```{r}
NCSS.ls <- readNCSS(dataDir = file.path(dataDownloadDir, 'NRCS_NCSS_20230922'),
...
#NCSS_lvl1 <- list(
  primary <-  NCSS.ls$long |> 
    ##nrow 3 710 257
    # use the column selection to subset the database
    inner_join(NCSS.ls$annotation |>
                 mutate(study_id = 'NCSS', table_id = NA) |>
                 #pull the annotations for the primary data entries
                 right_join(ISCN4map,
                           by = join_by(study_id, table_id,
                                        of_variable == source_variable)) |>
               ...
               relationship = "many-to-many",
               by = join_by(table_id, column_id)) |>
    select(study_id, table_id, column_id,  dataset_id, profile_id, layer_id, 
           of_variable, target_variable, is_type, with_entry) |>
    #nrow 1034386
    #TODO: Worldwide soil carbon and Northern Circumpolar needs to be address in readISCN
    #Removing them for now
    ...
    mutate(with_entry = if_else(is_type != 'value', 
                                paste(column_id, with_entry, sep = ':'), 
                                with_entry)) |>
   #### Remove duplicate observations from NCSS ####
   unique() |> #some entries repeated due to soc_id replication of non SOC variables
   #### Kick out to wide format for QAQC ####
    select(study_id, study_id, dataset_id, profile_id, layer_id,, #_id are the row identifier
           target_variable, is_type, with_entry) |>
...
#memory management
primary_variables <- c(primary_variables, 'NCSS_lvl1')
rm(list = setdiff(ls(), primary_variables)) 
```
...
ISCN4_primary <- ISCN3_lvl1$primary |>
  ...
  FIA_lvl1$primary |>
    ...
    mutate(`latitude::method` = 'NAD83') #This applies for non Pacific Island locations, Pacific Island locations are WSG84
  ) |>
  # country/state
  ...
  mutate(datum = str_remove(datum, pattern = 'datum \\(datum\\)\\:')) |>
