...
ISCN4map <- read_csv(file = file.path(annotationsDir,
                                      'ISCN4_map.csv'))
...
# Take the primary data in the long format and replace the table and column ids
# ...with the unique study_id-of_variable instead



ISCN3_lvl1 <- list(
  primary = ISCN_lvl0.ls$long |> 
    #slice_sample(n=10) |> #make the data smaller for debugging and development
    mutate(study_id = 'ISCN3') |>
    rename(dataset_id = dataset_name_soc, 
           site_id = site_name,
    ...
    right_join(ISCN_lvl0.ls$annotation |>
    ...
    select(column_id, study_id, dataset_id, profile_id, layer_id, 
