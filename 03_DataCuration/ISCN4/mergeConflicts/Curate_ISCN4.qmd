...
kable(ISCN_lvl0.ls$pivotBind |>
        slice(500:510))
```

```{r}
#| code-summary: "Create ISCN3 lvl 1 product (with subset)"
...
ISCN3_column_meta <- ISCN_lvl0.ls$annotation |>
  #pull the annotations for the primary data entries
  right_join(ISCN4map,
             by = join_by(study_id,# table_id,
                          of_variable == source_variable)) |>
  filter(with_entry != '--') |>
  select(ends_with('_id'), of_variable, target_variable, is_type, with_entry)

ISCN3_lvl1_pivotBind <- ISCN_lvl0.ls$pivotBind |> 
    ...
    dplyr::rename(dataset_id = dataset_name_sub, 
           site_id = site_name,
    ...
    inner_join(ISCN3_column_selection, 
               relationship = "many-to-many",
               by = join_by(study_id, table_id, column_id)) |>
    ##nrow 1 092 475
    select(study_id, dataset_id, table_id,
           profile_id, layer_id, column_id, row_id,
           of_variable, target_variable, 
           is_type, with_entry) |>
    ##nrow 1 092 546
    bind_rows(ISCN3_column_meta)

#memory management
primary_variables <- c(primary_variables, 
                       'ISCN3_lvl0.ls',  'ISCN3_lvl1_pivotBind')
...
FIA_column_selection <- FIA_lvl0.ls$annotation |>
  #pull the annotations for the primary data entries
  right_join(ISCN4map,
             by = join_by(study_id,
                          of_variable == source_variable),
...
FIA_meta <- FIA_lvl0.ls$annotations |>
    #pull the annotations for the primary data entries
    right_join(ISCN4map,
               by = join_by(study_id,
                            of_variable == source_variable),
...
FIA_lvl1_pivotBind <- FIA_lvl0.ls$joinPivot |>
    #### pull in the target variable that we want to map ####
    right_join(FIA_column_selection,
               relationship = "many-to-many",
    ...
    mutate(with_entry = if_else(is.na(keyed_value), 
                                with_entry, keyed_value)) |>
    select(-keyed_value) |>
    ...
    #nrow 7644100
  unique() |> # removing duplications from table joins
    #nrow 364253,
  bind_rows(FIA_meta)
...
# TODO go back and pull the citations, references and other study information
#ISCN4_study <- 
#
ISCN4_geolocation <- ISCN3_lvl1_pivotBind |>
  filter(of_variable %in% c('latitude', 'longitude', 'datum', 
                            'state', 'country', 'observation_year'))

temp_annotations <- ISCN3_lvl1_pivotBind |>
  filter(of_variable %in% c('latitude', 'longitude', 'datum', 
                            'state', 'country', 'observation_year'),
         is.na(dataset_id)) |>
  select(-one_of(c('dataset_id', 'study_id', 'profile_id', 'layer_id', 'row_id')))

temp_site <- ISCN3_lvl1_pivotBind |>
  filter(of_variable %in% c('latitude', 'longitude', 'datum', 
                            'state', 'country', 'observation_year'),
         is.na(layer_id)) |>
  select(-'layer_id') |>
  rename(site_row_id = row_id)

temp <- full_join(temp_annotations |>
                    filter(table_id == 'site'), temp_site)

temp_layer <- ISCN3_lvl1_pivotBind |>
  filter(!is.na(layer_id)) |>
  rename(layer_row_id = row_id)
